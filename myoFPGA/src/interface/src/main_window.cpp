...
bool stopJointSynchronization = false;
...
*****************************************************************************/
            ...
            : QMainWindow(parent)
    {
        ...
        QObject::connect(ui.button_start_dance_1, SIGNAL(clicked()), this, SLOT(startDance1()));
...
*****************************************************************************/
    ...
    void MainWindow::JointStatus(const roboy_communication_middleware::JointStatus::ConstPtr &msg){
        if (stopJointSynchronization) return;
        handleDance1();
            ...
            for (uint joint = 0; joint < NUMBER_OF_JOINT_SENSORS; joint++) {
                ...
                switch (joint) {
                    ...
                    case 1:{
                        ...
                        float result = pterm + dterm + integral[joint] - 100;
                        if (result <= -smooth_distance) {
...
//                    ROS_INFO_THROTTLE(1,"\npterm %f\ndterm %f\niterm %f\nresult %f", pterm, dterm, integral[joint], result);
    ...
    void MainWindow::submitWaitJoints() {
        stopJointSynchronization = true;
        ui.wait_button_joint_control->setStyleSheet("background-color: inherit");
        ui.stop_button_jointControl->setStyleSheet("background-color: blue");
    }

    void MainWindow::submitRunJoints() {
        stopJointSynchronization = false;
        ui.wait_button_joint_control->setStyleSheet("background-color: inherit");
        ui.stop_button_jointControl->setStyleSheet("background-color: blue");
    }

    void MainWindow::setActivatedButtonStyle(QPushButton * button) {
        button->setStyleSheet("background-color: blue");
    }

    void MainWindow::setDeactivatedButtonStyle(QPushButton * button) {
        button->setStyleSheet("background-color: inherit");
    }
    ...
    void MainWindow::handleDance1() {
        // ROS_INFO("FOUND t0: %f", t0.toSec());
        if (t0.toSec() == 0) return;

        int notRequiredDummyValue = 0;
        int strength = 30;
        // first step should be if t1 is not NULL and t2 is NULL
        if (currentDanceStep == 0) {
            setActivatedButtonStyle(ui.button_start_dance_1);
            ROS_INFO("DANCE1: Start dancing");

            // first part: move to the left
            // what to do: Make angle 2 positive and 1 negative
            ROS_INFO("DANCE1: Move to the left");
            ui.joint1->setValue(ui.joint1->value() - strength);
            ui.joint2->setValue(ui.joint2->value() + strength);
            updateSetPointsJointControl(notRequiredDummyValue);

            t1 = ros::Time::now();
            currentDanceStep += 1;
        } else {
            double diff = (ros::Time::now() - t0).toSec() * 1000;
            // ROS_INFO("FOUND DIFF OF %f", diff);
            int sleep = 1200;
            int timeOffset = 200;

            if (currentDanceStep == 4 && (sleep*4 - timeOffset) < diff && diff < (sleep*4+timeOffset)) {
                // 5. part RESET
                ROS_INFO("DANCE1: Finish");
                t0 = ros::Time(0);
                t1 = ros::Time(0);
                setDeactivatedButtonStyle(ui.button_start_dance_1);
                currentDanceStep += 1;
            } else if (currentDanceStep == 3 && (sleep*3 - timeOffset) < diff && diff < (sleep*3+timeOffset)) {
                // 4. part: move to the middle
                ROS_INFO("DANCE1: Move to the middle");
                ui.joint1->setValue(ui.joint1->value() - strength);
                ui.joint2->setValue(ui.joint2->value() + strength);
                updateSetPointsJointControl(notRequiredDummyValue);
                currentDanceStep += 1;
            } else if (currentDanceStep == 2 && (sleep*2 - timeOffset) < diff && diff < (sleep*2+timeOffset)) {
                // 3. part: move to the right
                ROS_INFO("DANCE1: Move to the right");
                ui.joint1->setValue(ui.joint1->value() + strength);
                ui.joint2->setValue(ui.joint2->value() - strength);
                updateSetPointsJointControl(notRequiredDummyValue);
                currentDanceStep += 1;
            } else if (currentDanceStep == 1 && (sleep - timeOffset) < diff && diff < (sleep+timeOffset)) {
                // snd part: move to the middle
                ROS_INFO("DANCE1: Move to the middle");
                ui.joint1->setValue(ui.joint1->value() + strength);
                ui.joint2->setValue(ui.joint2->value() - strength);
                updateSetPointsJointControl(notRequiredDummyValue);
